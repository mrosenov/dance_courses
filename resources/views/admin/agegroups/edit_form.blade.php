...
@section('section')
    ...
    <div class="card card-default">
        <div class="card-body">
            <form method="POST" action="{{route('update-age-group', $agegroup->id)}}">
                @csrf
                @method('PATCH')
                <div class="form-group">
                    <label for="Name">Name</label>
                    <input name="name" type="text" class="form-control" id="Name" placeholder="Name" value="{{$agegroup->name}}" required>
                </div>
                <div class="form-group">
                    <label for="Description">Description</label>
                    <textarea name="description" id="summernote">{{$agegroup->description}}</textarea>
                </div>
                ...
                <fieldset class="border p-2">
                    ...
                    <div class="custom-control custom-checkbox d-inline-block">
                        <input name="active" type="checkbox" class="custom-control-input" id="DanceActive" value="1" @if($agegroup->active) checked @endif>
